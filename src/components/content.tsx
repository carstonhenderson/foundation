 ...
 */
let Content = ({ children, className = '', withImage, reverse, withBackground }: ContentProps & WithImageProps): ReactElement => {
	if (withImage) {
		return (
		...
		return (
			<div style={background} className={`bg-cover bg-no-repeat ${bgClass} ${className}`}>
				<div className="w-full max-w-screen-xl mx-auto p-4 md:p-8 md:p-8">{children}</div>
			</div>
		)
	} else {
